...
int main( int argc, char* argv[] )
{
  ...
  TH1I hpixph( "pixph", "pixel ADC PH;pixel ph [ADC];pixels in clusters", 200, 0, 200 );
  TH1I hcolph( "colph", "column ADC PH;column ph [ADC];columns", 200, 0, 200 );
  ...
  TH1D hdycqx1( "dycqx1", "triplet residual, col < 2;triplet residual [um];col < 2 Q peak triplets",
		200, -100, 100 );
  TH1D hdycqx2( "dycqx2", "triplet residual, 2 < col < 30;triplet residual [um];2 < col < 30 Q peak triplets",
		200, -100, 100 );
  TH1D hdycqx3( "dycqx3", "triplet residual, 30 < col < 50;triplet residual [um];30 < col < 50 Q peak triplets",
		200, -100, 100 );
  ...
  TH1D hdycqm1( "dycqm1", "triplet residual, ymod < 7;triplet residual [um];ymod < 7 Q peak triplets",
		200, -100, 100 );
  TH1D hdycqm2( "dycqm2", "triplet residual, 7 < ymod < 8;triplet residual [um];7 < ymod < 8 Q peak triplets",
		200, -100, 100 );
  TH1D hdycqm3( "dycqm3", "triplet residual, 8 < ymod < 10;triplet residual [um];8 < ymod < 10 Q peak triplets",
		200, -100, 100 );
  TH1D hdycqm4( "dycqm4", "triplet residual, 10 < ymod < 11;triplet residual [um];10 < ymod < 11 Q peak triplets",
		200, -100, 100 );
  TH1D hdycqm5( "dycqm5", "triplet residual, 11 < ymod < 12;triplet residual [um];11 < ymod < 12 Q peak triplets",
		200, -100, 100 );
  TH1D hdycqm6( "dycqm6", "triplet residual, 12 < ymod < 13;triplet residual [um];12 < ymod < 13 Q peak triplets",
		200, -100, 100 );
  ...
  while( evFile.good() && ! evFile.eof() && nev < Nev ) {
	...
	vpx.push_back(px); // comment out = no clustering
      ...
      */
      ...
      set <cluster,clusrt> colpx[155]; // per column
	...
	for( int col = colmin+1; col < colmax-2; ++col ) { // skip 1st and lst
	  ...
	  if( colvec[col+0].size() <= 2 &&
	    ...
	    hdyc.Fill( dy*25 ); // [um] 2.64

	    if( colvec[col+1].size() <= 2 )
	      hdyc2.Fill( dy*25 ); // [um] 2.20

	    double q1 = qvec[col+1]; // [ke]

	    madyvsq.Fill( q1, abs(dy)*25 );
	    nrowvsq.Fill( q1, colvec[col+1].size() );

	    if( q1 > 5 && q1 < 11 ) {

	      hdycq.Fill( dy*25 ); // [um] 2.60

	      if( qvec[col+0] > 5 &&
		  qvec[col+2] > 5 &&
		  qvec[col+0] < 11 &&
		  qvec[col+2] < 11 )
		hdycq2.Fill( dy*25 ); // [um] 2.56

	      dyvsym.Fill( ymod, dy*25 );
	      madyvsx.Fill( col+1, abs(dy)*25 );
	      madyvsy.Fill( ay, abs(dy)*25 );
	      madyvsym.Fill( ymod, abs(dy)*25 );
	      nrowvsym.Fill( ymod, colvec[col+1].size() );

	      if(      col+1 <  2 )
		hdycqx1.Fill( dy*25 );
	      else if( col+1 < 30 )
		hdycqx2.Fill( dy*25 ); // 2.2
	      else if( col+1 < 50 )
		hdycqx3.Fill( dy*25 ); // 2.1
	      else
		hdycqx4.Fill( dy*25 ); // 2.3

	      if(      ymod25 <  7 )
		hdycqm1.Fill( dy*25 ); // 1.91
	      else if( ymod25 <  8 )
		hdycqm2.Fill( dy*25 );
	      else if( ymod25 < 10 )
		hdycqm3.Fill( dy*25 ); // 3.2
	      else if( ymod25 < 11 )
		hdycqm4.Fill( dy*25 );
	      else if( ymod25 < 12 )
		hdycqm5.Fill( dy*25 );
	    ...
	    if( colvec[col+1].size() == 2 ) {
	      ...
	      heta.Fill( eta );
	      madyvseta.Fill( eta, abs(dy)*25 ); // flat

	      if( abs(eta) < 0.4 )
		hdyc2eta.Fill( dy*25 ); // [um] 2.85
	...
	for( int col = colmin+1; col < colmax-8; ++col ) { // skip 1st and lst
	  ...
	  if( colvec[col+0].size() <= 2 &&
	    ...
	    hduc.Fill( du*25 ); // [um] 648 1.84

	    if( colvec[col+4].size() <= 2 )
	      hduc2.Fill( du*25 ); // [um] 648 1.83

	    double q4 = qvec[col+4]; // [ke]

	    maduvsq.Fill( q4, abs(du)*25 );

	    if( q4 > 5 && q4 < 11 ) {

	      hducq.Fill( du*25 ); // [um] 648 1.72

	      if( qvec[col+3] > 5 &&
		  qvec[col+5] > 5 &&
		  qvec[col+3] < 11 &&
		  qvec[col+5] < 11 )
		hducq2.Fill( du*25 ); // [um] 648 1.70

	      duvsum.Fill( umod, du*25 );
	      maduvsx.Fill( col+4, abs(du)*25 );
	      maduvsy.Fill( au, abs(du)*25 );
	      maduvsum.Fill( umod, abs(du)*25 );
	    ...
	    if( colvec[col+4].size() == 2 ) {
	      ...
	      double eta = (a1-a0)/(a1+a0);
	      maduvseta.Fill( eta, abs(du)*25 ); // flat

	      if( abs(eta) < 0.4 )
		hduc2eta.Fill( du*25 ); // [um] 2.85
