...
void resolutionVSlandau_Pirr(TString function = "RMSself"){
  ...
  TString pitch = "25";
  float perc[Cuts]={0.20,0.50,0.70,0.85,1.};        
  for(int i=0; i<Angles; i++)  {
    ...
    Double_t intervals[Cuts] = {mu-width,mu+width,mu+3*width,mu+5*width,1000.};
  ...
  */
  ...
  for(int i=0; i<Angles; i++){
    ...
    ofstream myfile;
    myfile.open ("/home/zoiirene/Output/TextFiles/Ascan_resTree_"+detectorB+"_dphcutB"+ss_dphcut+"_"+function+"_"+label+"_"+ss_Angle[i]+"_noADD.txt");
    myfile << "A " << detectorA << "\n";
    myfile << labelA << "\n";
    myfile << "B " << detectorB<< "\n";
    myfile << labelB << "\n";
