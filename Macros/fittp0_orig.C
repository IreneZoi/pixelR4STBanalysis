...
void fittp0( string hs, double x1 = 1, double x9 = 0 )
{
  ...
  for( int ii = i1; ii <= i9; ++ii ) {
    ...
    jj++;
    if( h->GetBinContent(ii)-bg > 0.5*(nmax-bg) )
  ...
  Ndata = jj;
  double hwhm = xhwhm-xmax;
  if( hwhm < dx ) hwhm = dx; // narrow peak protection
  double nn = 3*nmax*hwhm/dx/G;
  ...
  tp0Fcn->SetParameter( 1, hwhm ); // width
  tp0Fcn->SetParameter( 2, 7.5 ); // nu
