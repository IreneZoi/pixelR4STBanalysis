...
void resolutionVSthr194n5p6Tree_RMS(TString func = "RMS")
{
  ...
  TString pitch[Angles] = {"25"};
  int i_dphcut[dphcuts] = {18};
  Double_t d_dphcut[dphcuts] = {18};
  Double_t d_dphcuterr[dphcuts] = {0};
  TString ss_dphcut[dphcuts] = {"18"};
  ...
  for(int j = 0; j < Angles; j++)
    {
        hdx3_clchargeABC90evR[j] =  GetCheckHists(&(res_map[j]), Angles, dphcuts,dphcut, Run[j],ss_dphcut,pitch, Hist,hdx3_clchargeABC90evR[j]);
  ...
  ofstream myfile;
  myfile.open ("/home/zoiirene/Output/TextFiles/Ascan_"+detectorB+"_5p6_"+func+"_"+ss_Angle[0]+"deg_rmsTree.txt");
  myfile << "A " << detectorA << "\n";
  myfile << labelA << "\n";
  myfile << "B " << detectorB<< "\n";
  myfile << labelB << "\n";
  ...
  for(int i=0; i<Angles; i++)	{
    auto it2 = res_map[0].find(std::make_pair(Run[0][0]+"_"+ss_dphcut[0]+"_"+pitch[0],Hist));
    if(it2  != res_map[0].end()){
      ...
      FitTH1(it2->second, &(Resolution[0][i]), &(ResolutionError[0][i]), ss_Angle[0]+"_"+ss_BiasVoltage[0]+"_"+ss_dphcut[i], detectorA, detectorB, detectorC,func );
  ...
  //////////////          RESOLUTION ///////////////////////
  if(print) cout << "*************    RMS: *************"  << endl;
  double freshres = 3.35;
  double freshres_err = 0.01;
  ofstream myfileUnf;

  myfileUnf.open ("/home/zoiirene/Output/TextFiles/AScanTreeCorr_"+detectorB+"_5p6_"+func+"_"+ss_BiasVoltage[0]+"_bestAngle_res.txt");
  ...
  Hist = "nrowB";
   ...
   ofstream myfile3; 
   myfile3.open ("/home/zoiirene/Output/TextFiles/ThrScan_clsizeB_"+detectorB+"_"+ss_BiasVoltage[0]+"_bestAngle_5p6.txt");
   myfile3 << "A " << detectorA << "\n";
   myfile3 << labelA << "\n";
   myfile3 << "B " << detectorB<< "\n";
   myfile3 << labelB << "\n";
