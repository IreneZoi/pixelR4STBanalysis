...
void resolutionVSthr194n5p6TreeEntries_RMS(TString func = "RMSself")
{
  ...
  ofstream myfile;
  myfile.open ("/home/zoiirene/Output/TextFiles/Ascan_"+detectorB+"_5p6_"+func+"_"+ss_Angle[0]+"deg_rmsTree_"+label+".txt");
  myfile << "A " << detectorA << "\n";
  myfile << labelA << "\n";
  myfile << "B " << detectorB<< "\n";
  myfile << labelB << "\n";
  ...
  for(int i=0; i<Angles; i++)	{
    auto it2 = res_map[0].find(std::make_pair(Run[0][0]+"_"+ss_dphcut[0]+"_"+pitch,Hist));
    if(it2  != res_map[0].end()){
      ...
      FitTH1(it2->second, &(Resolution[0][i]), &(ResolutionError[0][i]), ss_Angle[0]+"_"+ss_BiasVoltage[0]+"_"+ss_dphcut[i], detectorA, detectorB, detectorC,func,&(Percentage[0][i]) );
  ...
  myfileUnf.open ("/home/zoiirene/Output/TextFiles/AScanTreeCorr_"+detectorB+"_5p6_"+func+"_"+ss_BiasVoltage[0]+"_bestAngle_res_"+label+".txt");
