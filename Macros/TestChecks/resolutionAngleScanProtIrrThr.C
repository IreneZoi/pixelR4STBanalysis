...
void resolutionAngleScanProtIrrThr(TString name = "preliminary_ThrScanProtIrr", TString func = "RMS95", bool unfolding = false){
  
  TString inputDir="/home/zoiirene/Output/TextFiles/";
  TString outputDir="/home/zoiirene/Output/Plots/";
  int i_dphcut[dphcuts] = {8, 10, 11, 12, 13, 14, 15, 16, 17, 18, 20, 22, 25, 27, 30, 32, 35, 37, 40};
  TString ss_dphcut[dphcuts] = {"8", "10", "11", "12", "13", "14", "15", "16", "17", "18", "20", "22", "25", "27", "30", "32", "35", "37", "40"};
  
  TString filenames[dphcuts];
  TString filenames2[dphcuts];
  for(int i = 0; i < dphcuts; i++){
    filenames[i] = "Ascan_resTree_120i_dphcutB"+ss_dphcut[i]+"_RMS95_testThr.txt";
    filenames2[i] = "Ascan_clsizeB_120i_dphcutB"+ss_dphcut[i]+"__RMS95_testEntries.txt";
  }
  
  TString irr =  "#phi_{eq} = 2.1#times10^{15} cm^{-2}, proton, 5.6 GeV";
  double angle_0[dphcuts][anglesPirr2];
  double angleerr_0[dphcuts][anglesPirr2];
  double res_0[dphcuts][anglesPirr2];
  double reserr_0[dphcuts][anglesPirr2];
  double angle;
  ...
  double clsize_0[dphcuts][anglesPirr2];
  double clsizeerr_0[dphcuts][anglesPirr2];
  ...
  for(int i =0; i< dphcuts; i++){
    resolutionPlot[i] = new TGraphErrors(anglesPirr2,angle_0[i],res_0[i],angleerr_0[i],reserr_0[i]);
    ...
    */
    ...
    resolutionPlot[i]->GetYaxis()->SetRangeUser(0.,20.);
    if(logar) resolutionPlot[i]->GetYaxis()->SetRangeUser(1.,50.);
  ...
  TLegend* legFDB2 = new TLegend(0.2,0.7,0.4,0.85);
  ...
  for(int i =0; i< dphcuts; i++){
    clsizeolutionPlot[i] = new TGraphErrors(anglesPirr2,angle_0[i],clsize_0[i],angleerr_0[i],clsizeerr_0[i]);
    ...
    /*
    clsizeolutionPlot[i]->SetMarkerColor(kRed+i-10);
    clsizeolutionPlot[i]->SetLineColor(kRed+i-10);
    if(i>15){
      clsizeolutionPlot[i]->SetMarkerColor(kMagenta+4+i-dphcuts);
      clsizeolutionPlot[i]->SetLineColor(kMagenta+4+i-dphcuts);
    }
    */
  ...
  clsizePlot[0] = new TGraphErrors(anglesPirr2,angle_0,clsize_0,angleerr_0,clsizeerr_0);
