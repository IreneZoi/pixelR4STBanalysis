...
void testGain2()//TString hist1 = "clszBiii",TString hist2 = "clszBiii",TString run1 = "2743", TString label1 = "p-stop", TString run2 = "2832", TString label2 = "p-spray", bool norm = true)
{
...
bool dphcut = false;
  ...
  for(int k=0; k<hists; k++)
      ...
      if(k == 5 || k ==0 )
	{
	  ...
	  name = outputDir+"compare_threshold40_RMS_"+Run[0]+"_"+Label[0]+"_"+Hist[k];//+"_"+Hist;
		  ...
		  if(k == 5 || k ==0 )
		    {
		      ...
		      auto it2 = m_hists.find(std::make_pair(Run[i]+"_"+Label[j],Hist[k]));
		      if(dphcut) it2 = m_hists.find(std::make_pair(Run[i]+"_"+ss_dphcut[l]+"_"+Label[j],Hist[k]));
		      if(it2  != m_hists.end())
			{
			...
			cout << " not found " << Run[i]+"_"+Label[j] << " hist " << Hist[k] << endl;
		      }
		      myfile[k] << "\\\\ \n";
		      myfile[k] << "\\hline \n";
  ...
  TString labelA="Pspray_default_FDB, thr 40 ADC";
  TString labelB="Pstop_RD53Apads_FDB, 120V, thr 40 ADC";
  TString labelC="Pstop_default_FTH, thr 40 ADC";
  TString info = "beam energy 5.8 GeV";
  ...
  ofstream myfile2;
  myfile2.open ("/home/zoiirene/Output/TextFiles/Thrscan40_res_"+detectorB+"_"+Run[0]+".txt");
  myfile2 << "A " << detectorA << "\n";
  myfile2 << labelA << "\n";
  myfile2 << "B " << detectorB<< "\n";
  myfile2 << labelB << "\n";
