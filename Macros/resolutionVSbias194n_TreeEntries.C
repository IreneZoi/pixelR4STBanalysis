	...
	for(int i=0; i<BiasVoltages; i++)
	  {
		...
		FitTH1(it2->second, &(Resolution[j][i]), &(ResolutionError[j][i]), ss_Angle[j]+"_"+ss_BiasVoltage[i]+"_dphcutB"+ss_dphcut[k], detectorA, detectorB, detectorC,func,&(Percentage[i][j]),&(Min[i][j]),&(Max[i][j]) );
