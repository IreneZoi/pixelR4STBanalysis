...
void resolution_2775_TreeEntries(TString function = "RMS95")
{

  TString detectorA[Angles]={"148"};
  TString detectorB[Angles]={"150"};
  TString detectorC[Angles]={"163"};
  TString labelA[Angles]={"Pstop_RD53Apads_FDB, thr 12 ADC"};
  TString labelB[Angles]={"Pstop_default_FDB thr 24 ADC"};
  TString labelC[Angles]={"Pspray_RD53Apads_FDB, thr 12 ADC"};
  ...
  int i_dphcut[dphcuts] = {24};
  TString ss_dphcut[dphcuts] = {"24"};

  TString Label[comparisons] = {"straightTracksY_isoAandCandB_straightTracksX"};
  TString Hist = "dx3_clchargeABC90evR95"; 
  bool dphcut = true;
  ...
  hdx3_clchargeABC90evR =    GetCheckHists(&res_map, Angles, dphcuts,dphcut, Run,ss_dphcut,pitch, Hist,hdx3_clchargeABC90evR,true,"testEntries");
  ...
  for(int i=0; i<Angles; i++){
      ...
      myfile[i].open ("/home/zoiirene/Output/TextFiles/Ascan_rmsTree_"+detectorB[i]+"_dphcutB"+ss_dphcut[i]+"_"+function+".txt");
      ...
      auto it2 = res_map.find(std::make_pair(Run[i]+"_"+ss_dphcut[i]+"_"+pitch[0],Hist));
      if(it2  != res_map.end())
	{
	  ...
	  FitTH1(it2->second, &(Resolution[i]), &(ResolutionError[i]), ss_Angle[i], detectorA[i], detectorB[i], detectorC[i], function );
...
//  DrawTGraphWithErrorDouble(Angles, Angle, Resolution, ResolutionError, Hist,"2743", Label[0], "angleScan_dphcutB"+ss_dphcut[0]+"_rmsTree", "RMS 95% [#mum]",-7.,29.,0.,8., "angleScan", "Angle [degrees]"  );
  ...
  myfile2[i].open ("/home/zoiirene/Output/TextFiles/Ascan_resTree_"+detectorB[i]+"_dphcutB"+ss_dphcut[i]+"_"+function+"_testEntries.txt");
