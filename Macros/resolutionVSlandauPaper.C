...
void resolutionVSlandauPaper(TString function = "RMSself"){
  ...
  float perc[Cuts]={0.20,0.50,0.65,0.80,1.};
  ...
  if(print) cout << "Getting files "  << endl;
    ...
    parameters = fitlang("straightTracksY_isoAandCandB_straightTracksX/clphB");
    double mu = parameters.first;
    double width = parameters.second;
    cout << " mu " << parameters.first << endl;
    double intervals[Cuts]={mu-width,mu+width,mu+3*width,mu+5*width,1000.};
      for(int k = 0; k< Cuts; k++){
      //perc[k] = (float) (k+1)*0.20;
  ...
  TFile *MyFile = new TFile(inputDir+"dx3_landau_"+ss_Angle[i]+"_dphcut"+ss_dphcut+"_"+label+".root","RECREATE"); 
  //plot landaus
  ...
  for(int k = 0; k< Cuts; k++){
    ...
    TString phB_low,phB_high,phA_high,phC_high; 
    ...
    tree->Draw("dx3tree>>hdx3treeph","clphBiiitree>"+phB_low+"&&clphBiiitree<"+phB_high+"&&clphAiiitree<"+phA_high+"&&clphCiiitree<"+phC_high,"goff");
  ...
  */
  ...
  for(int i=0; i<Angles; i++){
    ...
    ofstream myfile;
    myfile.open ("/home/zoiirene/Output/TextFiles/Ascan_resTree_"+detectorB+"_dphcutB"+ss_dphcut+"_"+function+"_"+label+"_angle"+ss_Angle[i]+"_noADD.txt");
    myfile << "A " << detectorA << "\n";
    myfile << labelA << "\n";
    myfile << "B " << detectorB<< "\n";
    myfile << labelB << "\n";
  ...
  //  h_res_vs_charge->Draw("HITS L");
    ...
    TString name = outputDir+"Res_vs_clustercherge_ABC_"+function+"_A_"+detectorA+"_B_"+detectorB+"_C_"+detectorC+"_run"+Run[i];
    c2->SaveAs(name+".eps");
    c2->SaveAs(name+".png");
    c2->SaveAs(name+".pdf");
    c2->Delete();
